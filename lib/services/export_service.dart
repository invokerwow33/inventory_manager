...
import 'dart:html' as html; // Для веба
...
class ExportService {
  ...
  static Future<void> exportToCsv({List<Equipment>? equipmentList}) async {
    final List<Equipment> equipment = equipmentList ?? await DatabaseHelper.instance.getAllEquipment();
  ...
  static Future<void> exportToExcel({List<Equipment>? equipmentList}) async {
    ...
    if (bytes != null) {
      _downloadFile(bytes, 'equipment_export_${_timestamp()}.xlsx', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    }
  }
  // ============ Helper ============
